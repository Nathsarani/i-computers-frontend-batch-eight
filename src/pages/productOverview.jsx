...
export default function ProductOverview(){
    ...
    const [status, setStatus] = useState("loading");  //loading ,error,sucsess
    ...
    return(
        ...
        }
        {
            ...
            <div className="w-full h-[calc(100vh-100px)] flex flex-col lg:flex-row ">
                   <h1 className="text-4xl font-semibold lg:hidden text-center sticky top-0 bg-primary">{product.name}</h1>
                ...
                <div className="w-full lg:w-1/2 h-full p-10 flex flex-col gap-6">
                    ...
                    <h3 className="text-2xl text-secondary/80 flex items-center">
							<CgChevronRight/> {product.category}
						</h3>
                        {/* alternative names */}
						{product.altNames && product.altNames.length > 0 && (
							<h3 className="text-2xl text-secondary/80">
								{product.altNames.join(" | ")}
							</h3>
						)}
                        
						<p className="text-sm text-justify text-secondary/90  h-32 overflow-y-auto">
						  {product.description}
						</p>
